...
namespace Nez.Verlet
{
	...
	{
		public Tree(System.Numerics.Vector2 origin, int depth = 5, float branchLength = 70, float theta = 0.4f,
		            float segmentCoef = 0.95f)
		{
			var root = AddParticle(new Particle(origin)).Pin();
			var trunk = AddParticle(new Particle(origin + new System.Numerics.Vector2(0, 10))).Pin();

			var firstBranch = CreateTreeBranch(root, 0, depth, segmentCoef, new System.Numerics.Vector2(0, -1), branchLength, theta);
			AddConstraint(new AngleConstraint(trunk, root, firstBranch, 3));

			// animates the tree at the beginning
			var noise = 3;
			for (var i = 0; i < Particles.Length; ++i)
				Particles.Buffer[i].Position += (new System.Numerics.Vector2(Mathf.Floor(Random.NextFloat() * noise),
					Mathf.Floor(Random.NextFloat() * noise)));
		}


		Particle CreateTreeBranch(Particle parent, int i, int nMax, float segmentCoef, System.Numerics.Vector2 normal,
		                          float branchLength, float theta)
		{
			...
			if (i < nMax)
			{
				var aRot = Mathf.RotateAround(normal, System.Numerics.Vector2.Zero, -theta * Mathf.Rad2Deg);
				var bRot = Mathf.RotateAround(normal, System.Numerics.Vector2.Zero, theta * Mathf.Rad2Deg);
